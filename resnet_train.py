...
def main():
    ...
    if args.model_type == 'rnnpluscnn':
        ...
        model = OnlyRNN(num_class,
                        ...
                        rnn_model=args.rnn_model,
                        pool_type=args.pool_type).cuda()
    elif args.model_type == 'standard_rnnresnet':
        model = resrnn_v1.resnet18(num_class=num_class, pretrained=True).cuda()
    else:
    ...
    for i, (data, label) in enumerate(train_loader):
        ...
        for t in range(s):
            if t == 0:
                ...
                rnn_cell_info = [[None, None]]
            else:
                past_frame = data[:, t-1, :, :, :]
            next_frame = data[:, t, :, :, :]
            if args.model_type == 'standard_resnet':
                out = model(next_frame)
            elif args.model_type == 'standard_rnnresnet':
                out, rnn_cell_info = model(next_frame, rnn_cell_info)
            else:
